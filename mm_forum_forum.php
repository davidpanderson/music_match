...
echo '
    <p>
    <form action="mm_forum_forum.php" method="get" class="form-inline">
...
if (user_can_create_thread($user, $forum)) {
    show_button(
        "mm_forum_post.php?id=$id", tra("New thread"), tra("Add a new thread to this forum")
    );
}
...
echo "
    <p>".
    tra("This message board is available as an %1 RSS feed %2", "<a href=mm_forum_rss.php?forumid=$forum->id&setup=1>", "<img src=img/feed_logo.png></a>");
...
function show_forum($forum, $start, $sort_style, $user) {
    $page_nav = page_links(
        "mm_forum_forum.php?id=$forum->id&amp;sort=$sort_style",
        $forum->threads,
        THREADS_PER_PAGE,
        $start
    );
