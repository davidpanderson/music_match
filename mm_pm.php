...
function show_block_link($userid) {
    echo " <a href=\"mm_pm.php?action=block&amp;id=$userid\">";
...
function do_inbox($logged_in_user) {
    ...
    if (count($msgs) == 0) {
        ...
        echo "<form name=msg_list action=mm_pm.php method=post>
        ...
        foreach($msgs as $msg) {
            ...
            show_button("mm_pm.php?action=new&amp;replyto=$msg->id", tra("Reply"), tra("Reply to this message"));
            show_button("mm_pm.php?action=delete&amp;id=$msg->id&amp;$tokens", tra("Delete"), tra("Delete this message"));
            echo "</ul></td></tr>\n";
        }
        echo "
            <tr><td>
...
function do_read($logged_in_user) {
    ...
    echo "<tr><td></td><td>\n";
    echo "<a href=\"mm_pm.php?action=new&amp;replyto=$id\">".tra("Reply")."</a>\n";
    echo " &middot; <a href=\"mm_pm.php?action=delete&amp;id=$id\">".tra("Delete")."</a>\n";
    echo " &middot; <a href=\"mm_pm.php?action=inbox\">".tra("Inbox")."</a>\n";
    end_table();
...
function do_send($logged_in_user) {
    ...
    Header("Location: mm_pm.php?action=inbox&sent=1");
}
...
function do_block($logged_in_user) {
    ...
    echo "<form action=\"mm_pm.php\" method=\"POST\">\n";
    echo form_tokens($logged_in_user->authenticator);
    ...
    echo "<a href=\"mm_pm.php?action=inbox\">".tra("No, cancel")."</a>\n";
    echo "</form>\n";
    page_tail();
}
...
function do_delete_selected($logged_in_user) {
    ...
    Header("Location: mm_pm.php?action=inbox&deleted=1");
}
